...
// The real sidebar uses Segoe Fluent Icons (Windows 11) or Segoe MDL2 Assets
// (Windows 10) and falls back to plain Unicode. A browser cannot ask whether a
// local face exists, but it can measure: a private-use glyph is wider in the
// icon face than in the fallback face.
function detectGlyphFont() {
  try {
...
export default function Sidebar({ pages, active, onSelect, collapsed, onToggle, theme }) {
  ...
  const glyph = (fluent, fallback) => glyphFont
    ? <span className="glyph fluent" style={{ fontFamily: glyphFont }} aria-hidden="true">{fluent}</span>
